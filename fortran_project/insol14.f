C          PROGRAM BERGER 78 version 2014
C
      PROGRAM Example_1_1
      READ(5,*) PHI,T,IOPT,NEFF,NOBB,NOPP                               PAL01035
      CALL insol(PHI,T,IOPT,NEFF,NOBB,NOPP)
      END
      SUBROUTINE insol(PHI,T,IOPT,NEFF,NOBB,NOPP)
...
C
C     This program is interactive 
C       It computes astronomical elements and insolation for one date
C       It can be modified but be carefull and check after modifications made
C
c     XND = Number of days in year
C     Solar constant is in program at beginning of section 4 
C     Insolation is given in Wm-2                                       PAL00100
C                                                                       PAL00110
...
C                                                                       PAL00360
      ...
      PIRR=PIR/3600.0D0 
      XND=365.0D0                                                       PAL00390
      STEP=360.0D0/XND                                                  PAL00400
...
c                                                                       PAL00411
 ...
 5996 FORMAT(1X,'THIS PROGRAMME COMPUTES THE TOTAL DAILY IRRADIATION RECPAL00413
     ...
     .E IN THE YEAR (IN WM-2)')                                         PAL00415
...
C                                                                       PAL01680
      SS=1368.0D0                                                       PAL01690
      TAU=1.0D0                                                         PAL01700
      SF=TAU*SS/PI                                                      PAL01710
      SO=DSIN(XOB*PIR)                                                  PAL01720
      XL=PERH+180.0D0                                                   PAL01730
      WRITE(6,6003) PHI, SS, XND                                        PAL01740
 6003 FORMAT(1X,'LATITUDE =',F6.1, 3X, 'Solar constant=', F8.2, 
     *'  Number days in year=', F10.4)                                  PAL01750
...
C                                                                       PAL01790
C     IF(IOPT.EQ.2) GO TO 20                                            PAL01800
      IF(IOPT.EQ.2) THEN
        READ(5,*) MA,JA
        CALL calendar_date(MA,JA)
      ELSE
        READ(5,*) TLS
        CALL solar_date()
      ENDIF
      WRITE(6,6002) WW,DAYL                                             PAL01940
 6002 FORMAT(1X,'DAY INSOL. =',F9.4,1X,'WM(-2)',4X,'LENGTH DAY ='       PAL01950
     *,F6.2,1X,'HOURS')                                                 PAL01960
C                                                                       PAL02290
  101 WRITE(6,6100)                                                     PAL02300
 6100 FORMAT(/70('*')/)                                                 PAL02310
C                                                                       PAL02320
C     GO TO 100                                                         PAL02330
      STOP                                                              PAL02340
      END                                                               PAL02350
C                                                                       PAL01810
...
C                                                                       PAL01888
C     READ(5,*) TLS                                                     PAL01890
C                                                                       PAL01900
      CALL DAYINS(ECC,XL,SO,TLS,PHI,PIR,PI,TEST,SF,WW,DAYL)             PAL01910
      WRITE(6,6001) TLS                                                 PAL01920
 6001 FORMAT(1X,'TRUE LONG. SUN =',F9.3)                                PAL01930
      END
C     GO TO 101                                                         PAL01970
C                                                                       PAL01980
C  20 CONTINUE                                                          PAL01990
C                                                                       PAL02000
...
C                                                                       PAL02080
     ...
     1DSIN(2.0D0*RANM)+13.0D0/12.0D0*XEC*DSIN(3.0D0*RANM)               PAL02220
      ...
      WRITE(6,6010) MA,JA,TLS,DLAMM                                     PAL02260
 6010 FORMAT(1X,'MONTH =',I3,3X,'DAY =',I3,3X,'TLS =',F9.3,'   DLAM=',
     * F9.3)                                                            PAL02270
      END
...
C                                                                       PAL02500
C   OUTPUT : WW=LY/DAY  OR  KJ M(-2) DAY(-1) or Wm-2
C            DAYL=LENGTH OF DAY (HOURS)                                 PAL02510
C                                                                       PAL02520
